...
if opts.lazyload then
		vim.api.nvim_create_autocmd({ "BufNewFile", "BufRead", "TabEnter", "TermOpen", "BufWinEnter", "WinClosed", "WinNew", "VimEnter" }, {
		pattern = "*",
		...
		callback = function()
			if #vim.fn.getbufinfo({ buflisted = 1 }) >= 2 or #vim.api.nvim_list_tabpages() >= 2 then
				...
				vim.opt.tabline = "%!v:lua.require('better-window.tabufline.modules').run()"
